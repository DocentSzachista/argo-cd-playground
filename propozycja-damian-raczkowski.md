...
```
Poniższy plik dodałbym w ścieżce `/etc/kubernetes/podsecurity-admission.yaml` dla zastosowania polityk bezpieczeństwa.
...
Przykładowy AppProject z zastosowaniem `syncWindow` w czasie godzin biznesowych (9-17 czasu polskiego) zamieściłem [tutaj](https://github.com/DocentSzachista/argo-cd-playground/blob/master/projects/my-project.yaml)
...
Ten plik został równiez zamieszczony [tutaj](https://github.com/DocentSzachista/argo-cd-playground/blob/master/applications/apps/hello-world.yaml)


Dla automatycznego wdrażania nowo dodanych obiektów `Applications` stworzyłbym plik `app-of-apps.yaml` który zawierałby obiekt `applications` który wskazywałby na folder `apps` w repozytorium. Dzięki temu możliwe by było dodawanie z automatu nowych aplikacji bez konieczności wywoływania komend `kubectl` czy `argocd`. Przykładowy manifest stworzyłem [tutaj](https://github.com/DocentSzachista/argo-cd-playground/blob/master/applications/app-of-apps.yaml).
...
## Linki
- repozytorium na githubie gdzie zamieściłem tą dokumentację: [link](https://github.com/DocentSzachista/argo-cd-playground)
- Link do repozytorium z drugim zadaniem: [link](https://github.com/DocentSzachista/cdq-recruitment-task)